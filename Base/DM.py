...
class BaseDm(object):
    ...
    def FreeReg(self, reg_code, ver_info):
        dmreg = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dll', 'DmReg.dll')
        location_dmreg = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dll', 'dm.dll')
    ...
    def CrackReg(self, dm_path):
        dmreg = os.path.join(dm_path, '../dll/DmReg.dll')
        location_dmreg = os.path.join(dm_path, '../dll/dm.dll')
        dms = ctypes.windll.LoadLibrary(dmreg)
        dms.SetDllPathW(location_dmreg, 0)
        self.dm = CreateObject('dm.dmsoft')
        return self.dm
